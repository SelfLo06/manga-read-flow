...
| --- | --- | --- | --- |
| P0-1 | Workflow State / Workflow Loop 详细设计 | `docs/design/workflow-state/` | 状态转移表、retry budget 消耗、恢复规则、stale 传播、pause/cancel/recover 语义。 |
| P0-2 | Provider Adapter 接口设计 | `docs/design/provider-adapter/` | Detector/OCR/Translation/Cleaner/Typesetter 的输入输出 schema、错误规范、capability metadata、license metadata。 |
| P0-3 | ArtifactService 详细设计 | `docs/design/artifact-service/` | artifact 目录布局、临时文件、原子写入、hash、retention、trash、missing 修复、debug artifact 策略。 |
| P0-4 | QualityCheckService / IssueType 详细设计 | `docs/design/quality-check/` | 质量检查规则、IssueType、severity、blocking 判定、用户提示文案、root stage 归因。 |
| P0-5 | API 设计 | `docs/design/api/` | Project/Batch/Page/TextBlock/Task/Review/Export 路由、DTO、错误响应、文件预览接口。 |
| P0-6 | MVP 垂直切片实现计划 | `docs/implementation/mvp-slice/` | 单 Project、单 Batch、单 Page 的端到端任务拆分、FakeProvider 到真实 Provider 的替换路径。 |
...
| Quality Gate | `docs/design/quality-check/` | IssueType taxonomy、severity/blocking matrix、message catalog。 |
| Provider Adapter | `docs/design/provider-adapter/` | 五类 Provider interface、DTO、capability metadata、错误映射。 |
| ArtifactService | `docs/design/artifact-service/` | 目录布局、原子写入、hash、retention、trash、cleanup、missing 检测。 |
| Storage | `docs/design/data-model/` | DDL、ORM、migration、repository methods。 |
...
| Export | `docs/design/export/` | ExportRecord、manifest schema、warning export、blocked export 行为。 |
| Testing | `docs/testing/architecture-validation/` | FakeProvider、crash recovery、export gate、artifact lifecycle 测试。 |
