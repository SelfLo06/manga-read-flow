...
## 6. Capability Closure Loop

设计、实验和产品实现不得长期作为互不相干的交付阶段。同一个 capability 可以循环进行最小合同核实、必要 Spike 和正式实现，但每轮工作都必须朝同一正式 extension point 和 direct downstream 收敛。

Spike 仍保持隔离：不登记正式 artifact，不更新 active pointer，不创建正式 QualityIssue，不改变 Workflow 决策。每个 Spike 开始前必须声明 Productization Exit：成功后进入哪个正式组件，失败后阻断哪个下游，以及哪些实验事实不会成为产品 source of truth。

Spike 得到 `GO` 或 `CONDITIONAL_GO` 后，应优先实现已证明的正式范围，而不是立即转向另一个横向实验。能力进入下一产品边前至少达到：

```text
PRODUCT_IMPLEMENTED
+ CANONICAL_PATH_INTEGRATED
+ CHECK_WORKFLOW_ACCEPTED
+ DIRECT_DOWNSTREAM_VERIFIED
```

禁止用冻结实验输出长期替代真实上游，在上游只有 FakeProvider 时宣称下游产品完成，为端到端演示创建 shadow pipeline，以 Web/UI 旁路正式 application/service，或用 helper/runner 测试代替正式入口证据。本节约束工作组织和产品化退出时机，不改变既有 Design Binding、Spike 隔离、Workflow 或主链路验证语义。
