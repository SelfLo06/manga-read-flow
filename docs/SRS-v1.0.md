...
系统第一优先级支持日语到简体中文。英语到简体中文作为后续扩展，不纳入 MVP 第一阶段。
...
检测结果不保证覆盖所有文字区域。低置信度或复杂区域应标记为待校对或跳过。
...
MVP 只要求提供启发式排序，不保证复杂分镜、跨栏对话、插入语和气泡嵌套场景完全正确。
...
MVP 默认采用云端 LLM API 以保证翻译质量，同时保留本地模型 fallback 接口。
...
MVP 可以只实现接口和手动切换，自动 fallback 可作为 P1。
...
系统应优先保证译文可读、尽量不溢出，而不是追求专业排版美观。

### FR-TY-003 竖排文本处理

系统应记录原文本方向。

对于原竖排日文，MVP 默认允许使用基础中文横排嵌字。

基础竖排中文嵌字作为 P1/P2，不作为 MVP 成功必要条件。
...
* 手动调整阅读顺序；
* LaMa 清字增强；
* 本地 SakuraLLM / Qwen / Gemma fallback；
* 基础排版调整；
* 字体选择；
* 术语表快捷添加；
* 翻译成本提示；
...
* 系统可以对普通对话框或旁白框中的日文文本执行 OCR；
* 系统可以保存 OCR 工具、模型来源、输入裁剪图和结果；
* 用户可以查看和修改 OCR；
* 单个 TextBlock OCR 失败不导致整页失败；
* OCR 置信度缺失时，系统提供质量标记。

### 11.4 翻译验收

* 系统可以按 Page 上下文将 OCR 结果翻译为中文；
* 翻译时可以注入 Project 术语表；
* 系统可以检查译文是否为空、是否缺块、是否未翻译、是否术语不一致；
* 系统可以在候选译文不合格时进行有限自动重译；
* 系统可以将无法自动修正的译文标记为 needs_review；
* 用户可以修改译文；
* 用户可以锁定译文；
* 云端 Provider 拒绝时，系统记录失败原因并允许 fallback 或人工处理。

### 11.5 清字验收

* 系统可以对普通白底或浅色气泡执行基础清字；
* 系统可以保存清字 mask 和清字后图片；
* 清字结果不覆盖原图；
* 复杂背景文字可跳过且不阻塞整页；
* 用户可以重试清字或跳过 TextBlock。
...
* 系统支持自动换行和字号自适应；
* 系统尽量避免文字溢出；
* 最小字号仍溢出时，系统标记 typeset_overflow；
* 用户可以修改译文后重新排版。

### 11.7 中断恢复验收

* 处理中断后，系统保留已完成结果；
* 重新打开 Batch 后，可从未完成步骤继续；
* 已完成步骤不应无故重复调用外部工具；
* 单页或单块失败不影响其他成功结果；
* 任务日志中可追踪失败原因。
...
* 30–50 页日语漫画样本；
* PaddleOCR 检测召回与误检；
* manga-ocr 对 crop 的识别准确率；
* 单 Page 翻译 JSON 稳定性；
* Project 术语表对人名和术语一致性的改善；
* 翻译质量检查与重译 loop；
* 白底填充 + OpenCV 清字效果；
* Pillow 自动换行和字号自适应；
* 中断恢复与避免重复调用；
* 云端翻译拒绝时的错误记录与 fallback。