...
MODULE_PATH = Path("tools/spikes/cleaning/physical_boundary/evidence.py")
...
def test_algorithm_has_no_case_or_target_identifier_branch():
    tree = ast.parse(Path("tools/spikes/cleaning/physical_boundary/evidence.py").read_text(encoding="utf-8"))
...
def test_frozen_case71_controls_cannot_expand_or_cross_hard_barriers():
    ...
    case_root = Path("data/local/mvp1-full-page-cleaning-v0.1/slice-3/run-v0.2/case-71")
    summary = json.loads((case_root / "summary.json").read_text(encoding="utf-8"))
    bgr = cv2.imread(summary["source"]["path"], cv2.IMREAD_COLOR)
